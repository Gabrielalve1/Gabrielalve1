...
const defaults={goal:4000,salary:0,saveGoal:0,reservePct:0,weekdays:[1,2,3,4,5],bills:[]};
...
function calculate(s,m,today=dateKey()){
 ...
 const scheduled=date=>date>=p.start&&p.weekdays.includes(new Date(date+'T12:00:00').getDay());
 const remaining=dates.filter(date=>date>=today&&(scheduled(date)||s.days[date]?.status==='open')&& !['closed','rest'].includes(s.days[date]?.status));
 const gross=sum(worked,'gross'),cost=cash(sum(worked,'fuel')+sum(worked,'other')),net=cash(gross-cost),reserve=cash(Math.max(0,net)*p.reservePct/100);
 ...
 const projection=closed.length?cash(gross+remaining.reduce((a,date)=>a+Math.max(0,avg-(s.days[date]?.gross||0)),0)):null;
 const projectedNet=closed.length?cash(net+remaining.reduce((a,date)=>{const r=s.days[date];return a+(r?Math.max(0,avgNet-(r.gross-r.fuel-r.other)):avgNet)},0)):null;
 ...
 const available=cash(p.salaryReceived+extra+net-reserve-personalCost-bills),canSave=cash(Math.min(p.saveGoal,Math.max(0,available))),free=cash(available-canSave);
 ...
 return{p,dates,days,worked,closed,remaining,gross,cost,net,reserve,rem,todayGross,goalToday,avg,avgNet,projection,projectedNet,extra,personalCost,bills,paid,available,canSave,free,projectedAvailable,missed,hours,km,trips:sum(worked,'trips'),scheduled};
}
function validate(s){
